...
const PostCard: FC<PostCardProps> = ({ post, isWithCompletedInfo }) => {
  if (isWithCompletedInfo) {
    return (
      <Link href={`/${post.slug}`}>
        <div className="w-96 flex flex-col justify-between bg-white rounded-lg shadow-lg hover:cursor-pointer overflow-hidden ">
          <div className='h-[200px] w-[400px]'>
            <Image src={post.data.image} alt={post.data.title} width="400px" height="200px" />
          </div>
          <div className='p-4 flex flex-col justify-between h-[40%]'>
            <h3 className='text-xl text-black'>{post.data.title}</h3>
  ...
  return (
    <Link href={`/${post.slug}`}>
      <div className="w-80 p-4 gap-4 flex flex-col justify-between bg-white rounded-lg  border-0 shadow-lg border-gray-200  hover:cursor-pointer overflow-hidden ">
        {/* <div className='h-[60%]'>
